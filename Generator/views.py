    ...
    # POSTing to the form means the request body will have some data
        ...
        if 'type' in request.POST and 'sk1' in request.POST and 'sk2' in request.POST \
            ...
            if is_valid(request):
                ...
                sk1_valid_levels = get_valid_levelrange(request.POST['sk1'], int(request.POST['sk1_min']),
                                                        int(request.POST['sk1_max']))
                sk2_valid_levels = get_valid_levelrange(request.POST['sk2'], int(request.POST['sk2_min']),
                                                        int(request.POST['sk2_max']))
                # If range for skill 1 is invalid, inform user
                if not sk1_valid_levels[0]:
                    messages.info(request, _("Invalid skill range: ") + request.POST['sk1'] + _(
                        ". This skill only has the following levels: ") + str(sk1_valid_levels[1]))
                    return render(request, 'form.html', {})
                # If range for skill 2 is invalid, inform user
                elif not sk2_valid_levels[0]:
                    messages.info(request, _("Invalid skill range: ") + request.POST['sk2'] + _(
                        ". This skill only has the following levels: ") + str(sk2_valid_levels[1]))
                    return render(request, 'form.html', {})
                else:
                ...
                messages.info(request,
                              _("Request invalid. Please check the SFIA skill codes to ensure the codes you entered are valid"))
                return render(request, 'form.html')

        # If data was posted from the search function form.
        elif 'input' in request.POST:
            messages.info(request,
                          _("The form has been pre-populated with the search results. If no suitable skills have been found, the fields have been left blank."))
...
def list_skills(request):
    ...
    return render(request, 'list_skills.html', {"set_1": set_1, "set_2": set_2,
                                                "set_3": set_3,
                                                "category_skill": category_skill})  # Renders and returns the page of the list of skills


# categorize skills

def category_skills():
    object = en_Skill.objects.all().order_by('category')
    list1 = []
    for skill in object:
        list1.append(skill.category)
    category = list(set(list1))
    dict0 = {}
    dict1 = {}
    dict2 = {}
    dict3 = {}
    ...
    for x in category:
        ...
        subct = en_Skill.objects.filter(category=x)
        list4 = []
        for subcategory in subct:
            list4.append(subcategory.subcategory)
        subcategories = list(set(list4))
        for y in subcategories:
            dict2["name"] = y
            list5 = en_Skill.objects.filter(category=x, subcategory=y)
            for skill in list5:
                dict3["name"] = skill.name
                dict3["code"] = skill.code
                dict3["description"] = skill.description
                dict3_copy = copy.deepcopy(dict3)
                list3.append(dict3_copy)
            list3_copy = list3.copy()
            dict2["skill"] = list3_copy
            dict2_copy = copy.deepcopy(dict2)
            list3 = []
            list1.append(dict2_copy)
        list1_copy = list1.copy()
        dict1["subcategory"] = list1_copy
        dict1_copy = copy.deepcopy(dict1)
        list_c.append(dict1_copy)
    dict0["category"] = list_c
    return dict0


def get_skill_sets_category():
    set_1 = []  # Column 1
    set_2 = []  # Column 2
...
def generate(request):
    ...
    if welsh_docxtemplates_available():
        doc = docx.Document(
            settings.BASE_DIR + '/Generator/DocxTemplates/{formType}_template_cy.docx'.format(formType=type))
    else:
        doc = docx.Document(
            settings.BASE_DIR + '/Generator/DocxTemplates/{formType}_template_en.docx'.format(formType=type))
...
''' 
#this function is not needed in this version 
